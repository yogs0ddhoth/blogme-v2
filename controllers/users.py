...
from flask_jwt_extended import create_access_token, get_jwt, get_jwt_header, get_jwt_identity, jwt_required, unset_jwt_cookies
import sqlalchemy
...
def signup(): # req: {name:string, email:string, password:string}
  ...
  return jsonify(id=newUser.id, user=newUser.name, access_token=access_token)
...
def login(): # req: {email:string, password:string}
  ...
  session['loggedIn'] = True
  return jsonify(id=user.id, user=user.name, access_token=access_token)
